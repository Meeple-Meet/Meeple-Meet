...
echo "🔍 Checking for delay/Thread.sleep in Compose test files..."
echo ""
...
while IFS= read -r file; do
  ...
  if [ -n "$delay_matches" ]; then
    ...
    echo -e "${RED}✗ Delay found in: ${file#*/}${NC}"
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
if [ $ERRORS -eq 0 ]; then
  echo -e "${GREEN}✓ No delays detected in Compose test files!${NC}"
  exit 0
else
  echo -e "${RED}✗ Found delays in $ERRORS file(s) with Compose rules${NC}"
  echo ""
  ...
  echo "  ✅ composeTestRule.waitUntil(timeoutMillis = 5000) {"
