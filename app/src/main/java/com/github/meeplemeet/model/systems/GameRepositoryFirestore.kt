 ...
 *
 * This repository retrieves and filters [Game] objects from the "gamesCollection" collection in Firestore.
 * Each document in Firestore represents a [GameNoUid], with its document ID used as the [Game.uid].
 */
class GameRepositoryFirestore(db: FirebaseFirestore) : GameRepository {
    ...
    override suspend fun getGameById(gameID: String): Game {
        ...
        return game?.let { fromNoUid(gameID, it) }
    ...
    /**
     * Retrieves all games that include a given genre ID.
     *
     * This uses Firestore's `array-contains` query operator.
     */
    override suspend fun getGamesByGenre(genreID: Int): List<Game> {
        val query = gamesCollection.whereArrayContains("genres", genreID).get().await()
        return mapSnapshotToGames(query.documents)
    }

    /**
     * Retrieves all games that include **all** of the specified genre IDs.
     *
     * Since Firestore doesn't support multiple `array-contains` filters simultaneously,
     * this method performs the intersection manually on the client side.
     */
    override suspend fun getGamesByGenres(genreIDs: List<Int>): List<Game> {
        if (genreIDs.isEmpty()) return emptyList()

        // Fetch all games that match at least one of the genres (no multiple array-contains allowed on Firestore)
        ...
        return initialGames.filter { game ->
            genreIDs.all { it in game.genres }
        }
    }
