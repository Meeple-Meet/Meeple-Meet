...
class AuthRepoFirebaseTest {
    ...
    fun `registerWithEmail returns failure if Firestore set throws`() = runBlocking {
        ...
        whenever(mockAuth.createUserWithEmailAndPassword(email, password)).thenReturn(mockTaskAuthResult)
    ...
    fun `registerWithEmail returns failure if createUserWithEmailAndPassword fails`() = runBlocking {
        ...
        whenever(mockAuth.createUserWithEmailAndPassword(email, password)).thenReturn(failedTask(exception))
    ...
    @Test
    fun `registerWithEmail returns failure if createUserWithEmailAndPassword returns null user`() = runBlocking {
        val mockAuth = mock<FirebaseAuth>()
        val mockAuthResult = mock<AuthResult>()
        val mockFirestore = mock<FirebaseFirestore>()
        whenever(mockAuth.createUserWithEmailAndPassword(any(), any())).thenReturn(completedTask(mockAuthResult))
    ...
    fun `loginWithEmail returns success for valid credentials`() = runBlocking {
        ...
        whenever(mockAuth.signInWithEmailAndPassword(email, password)).thenReturn(completedTask(mockAuthResult))
        whenever(mockAuthResult.user).thenReturn(mockUser)
        whenever(mockUser.uid).thenReturn(uid)
        whenever(mockUser.email).thenReturn(email)
        whenever(mockUser.displayName).thenReturn(name)
    ...
    @Test
    fun `loginWithEmail returns failure if signInWithEmailAndPassword returns null user`() = runBlocking {
        val mockAuth = mock<FirebaseAuth>()
        val mockAuthResult = mock<AuthResult>()
        val mockFirestore = mock<FirebaseFirestore>()
        whenever(mockAuth.signInWithEmailAndPassword(any(), any())).thenReturn(completedTask(mockAuthResult))
        whenever(mockAuthResult.user).thenReturn(null)
        val repo = AuthRepoFirebase(auth = mockAuth, helper = mock(), firestore = mockFirestore)
    ...
    fun `loginWithGoogle returns failure if extractIdTokenCredential throws`() = runBlocking {
        ...
        whenever(mockHelper.extractIdTokenCredential(any())).thenThrow(RuntimeException("extract failed"))
    ...
    fun `loginWithGoogle returns failure if toFirebaseCredential throws`() = runBlocking {
        val mockHelper = mock<GoogleSignInHelper>()
        val mockIdTokenCredential = mock<com.google.android.libraries.identity.googleid.GoogleIdTokenCredential>()
        ...
        whenever(mockHelper.toFirebaseCredential(any())).thenThrow(RuntimeException("toFirebaseCredential failed"))
        val repo = AuthRepoFirebase(auth = mock(), helper = mockHelper, firestore = mock())
        val credential = CustomCredential(type = "google_id_token", data = android.os.Bundle())
        val result = repo.loginWithGoogle(credential)
        assertTrue(result.isFailure)
    ...
    fun `loginWithGoogle does not create user if Firestore doc exists`() = runBlocking {
        ...
        val credential = CustomCredential(
            type = com.google.android.libraries.identity.googleid.GoogleIdTokenCredential.TYPE_GOOGLE_ID_TOKEN_CREDENTIAL,
        ...
        whenever(mockAuth.signInWithCredential(mockFirebaseCredential)).thenReturn(completedTask(mockAuthResult))
        whenever(mockAuthResult.user).thenReturn(mockUser)
        whenever(mockUser.uid).thenReturn(uid)
        whenever(mockUser.email).thenReturn(email)
        whenever(mockUser.photoUrl).thenReturn(null)
