...
class SessionComponentsTest {
  ...
  fun discretePillSlider_all_paths_one_test() {
    ...
    var isoB = false
    var hits = 0
    var toggleEditable by mutableStateOf(true)
    var colorHit = false
    var lastPair: Pair<Float, Float>? = null
    ...
    composeRule.onAllNodesWithTag(ComponentsTestTags.PILL_RANGE_SLIDER)[0].performTouchInput {
      down(centerLeft)
      moveTo(centerRight)
      up()
    }

    composeRule.onAllNodesWithTag(ComponentsTestTags.PILL_RANGE_SLIDER)[1].performTouchInput {
      down(center)
      moveBy(Offset(50f, 0f))
      up()
    }

    composeRule.onAllNodesWithTag(ComponentsTestTags.PILL_RANGE_SLIDER)[2].performTouchInput {
      down(centerLeft)
      moveTo(centerRight)
      up()
    }
    ...
    composeRule
        .onNode(
            hasTestTag(ComponentsTestTags.PILL_RANGE_SLIDER) and
                hasAnyAncestor(hasTestTag("pillA-parent")))
        .performTouchInput {
          down(centerLeft)
          moveTo(centerRight)
          up()
        }
    composeRule.runOnIdle { assert(isoA && !isoB) }

    composeRule
        .onNode(
            hasTestTag(ComponentsTestTags.PILL_RANGE_SLIDER) and
                hasAnyAncestor(hasTestTag("pillB-parent")))
        .performTouchInput {
          down(centerLeft)
          moveTo(centerRight)
          up()
    ...
    // Toggle editable slider
    val toggleNode =
        composeRule.onNode(
            hasTestTag(ComponentsTestTags.PILL_RANGE_SLIDER) and
                hasAnyAncestor(hasTestTag("pillToggle-parent")))
    toggleNode.performTouchInput {
      down(center)
      moveBy(Offset(100f, 0f))
      up()
    }
    composeRule.runOnIdle { assert(hits >= 1) }
    composeRule.runOnUiThread { toggleEditable = false }
    ...
    composeRule.runOnIdle { assert(hits == before) }

    // Custom colors slider
    val colorNode =
        composeRule.onNode(
            hasTestTag(ComponentsTestTags.PILL_RANGE_SLIDER) and
                hasAnyAncestor(hasTestTag("pillColor-parent")))
    colorNode.performTouchInput {
      down(centerLeft)
      moveTo(centerRight)
      up()
    }
    composeRule.runOnIdle {
      assert(colorHit)
      val lp = lastPair ?: error("No slider callback received for color slider")
      assert(lp.first <= lp.second)
      assert(lp.first in -10f..10f && lp.second in -10f..10f)
    }

    composeRule.runOnIdle {
      val p = enabledPair ?: error("Enabled slider did not report values")
      assert(p.first <= p.second)
      assert(zeroStepsCalled)
      assert(!disabledCalled)
    }

    composeRule.onNodeWithTag("surround", useUnmergedTree = true).assertExists()
    composeRule.onNodeWithTag("host", useUnmergedTree = true).assertExists()
  }
