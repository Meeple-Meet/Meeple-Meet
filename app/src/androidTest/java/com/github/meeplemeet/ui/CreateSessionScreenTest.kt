...
class CreateSessionScreenTest {
    ...
    fun game_search_bar_header_and_clear_icon_present_and_working() {
        ...
        compose.waitUntil(3_000) {
            compose.onAllNodesWithContentDescription("Clear", useUnmergedTree = true)
    ...
    fun discard_from_screen_resets_fields_and_participants() {
        ...
        gameInput().assert(
            SemanticsMatcher.expectValue(
                SemanticsProperties.EditableText, AnnotatedString("Y")))

        locationInput().assert(
            SemanticsMatcher.expectValue(
                SemanticsProperties.EditableText, AnnotatedString("Z")))
