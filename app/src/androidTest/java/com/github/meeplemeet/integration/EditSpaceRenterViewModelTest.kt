...
class EditSpaceRenterViewModelTest {
  ...
  private val testOwner =
      Account(
          uid = "owner123", name = "Test Owner", email = "owner@test.com", handle = "test_owner")
  private val otherUser =
      Account(
          uid = "other456", name = "Other User", email = "other@test.com", handle = "other_user")

  private val testSpaceRenter =
      SpaceRenter(
          id = "renter123",
          owner = testOwner,
          name = "Test Space",
          ...
          spaces = emptyList(),
          photoCollectionUrl =
              listOf("https://example.com/old1.jpg", "https://example.com/old2.jpg"))
  ...
  fun updateSpaceRenter_updatesRepository_whenOnline() = runBlocking {
    ...
    assertTrue(
        fakeSpaceRenterRepository.lastUpdatedPhotos?.contains("https://example.com/old1.jpg") ==
            true)
    // The fake image repo turns "local/path/..." into
    // "https://example.com/spacerenter/renter123/photo_1.webp" approx
    // We can check if it contains *a* new url
    assertTrue(
        fakeSpaceRenterRepository.lastUpdatedPhotos?.any { it.contains("spacerenter") } == true)

    // Verify removed photo ("old2.jpg") is gone from the update payload
    assertTrue(
        fakeSpaceRenterRepository.lastUpdatedPhotos?.contains("https://example.com/old2.jpg") ==
            false)
  }
  ...
  // A manual fake allows us to bypass "IncompatibleClassChangeError" seen with Mockk+Lists
  // sometimes
