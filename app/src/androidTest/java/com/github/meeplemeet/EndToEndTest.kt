...
/**
 * End-to-end test for Meeple Meet application.
 * Tests the complete user journey from sign-up to navigating the app.
 */
...
class EndToEndTest : FirestoreTests() {
    ...
    fun completeUserJourney_signUpCreateAccountAndNavigate() {
        ...
        composeTestRule.onNodeWithTag(SignInScreenTestTags.SIGN_UP_BUTTON)
            .assertExists()
            .performClick()
        ...
        composeTestRule.onNodeWithTag(SignUpScreenTestTags.SIGN_UP_BUTTON)
            .assertExists()
            .assertIsEnabled()
            .performClick()

        // NEW: Wait for Create Account screen to appear (it uses text labels like "Handle", "Username", and a button "Let's go!")
        composeTestRule.waitUntil(timeoutMillis = 12_000) {
            try {
                composeTestRule.onAllNodesWithText("Let's go!", substring = true).fetchSemanticsNodes().isNotEmpty()
            } catch (_: Throwable) {
                false
            }
        }

        // Fill Create Account screen: provide a unique handle and username, then press the create button
        ...
        composeTestRule.onNodeWithText("Handle", substring = true)
            .assertExists()
        ...
        composeTestRule.onNodeWithText("Let's go!")
            .assertExists()
            .assertIsEnabled()
            .performClick()
        ...
        composeTestRule.waitUntil(timeoutMillis = 15_000) {
            try {
                composeTestRule.onAllNodesWithTag(NavigationTestTags.BOTTOM_NAVIGATION_MENU).fetchSemanticsNodes().isNotEmpty()
            } catch (_: Throwable) {
                false
            }
        }
        ...
        composeTestRule.onNodeWithTag(NavigationTestTags.DISCOVER_TAB)
            .assertExists()

        // Navigate to Sessions tab
        composeTestRule.onNodeWithTag(NavigationTestTags.SESSIONS_TAB)
            .assertExists()
            .performClick()
        ...
        composeTestRule.onNodeWithTag(NavigationTestTags.DISCUSSIONS_TAB)
            .assertExists()
            .performClick()
        ...
        composeTestRule.onNodeWithTag(NavigationTestTags.PROFILE_TAB)
            .assertExists()
            .performClick()
        ...
        // Note: account creation steps removed — the username may be blank if the app requires a separate flow.
        if (testUsername.isNotEmpty()) {
            try {
                composeTestRule.onNodeWithText(testUsername, useUnmergedTree = true)
                    .assertExists()
            } catch (_: Throwable) {
        ...
        composeTestRule.onNodeWithTag("Logout Button")
            .assertExists()
            .assertIsEnabled()
            .performClick()
        ...
        composeTestRule.onNodeWithTag(SignInScreenTestTags.SIGN_IN_BUTTON)
            .assertExists()
            .assertIsDisplayed()
    }
    ...
    fun twoUsers_createAndJoinDiscussion() {
        ...
        // Alice signs in
        composeTestRule.onNodeWithTag(SignInScreenTestTags.EMAIL_FIELD)
            .assertExists()
            .performTextInput(user1Email)
        composeTestRule.onNodeWithTag(SignInScreenTestTags.PASSWORD_FIELD)
            .assertExists()
            .performTextInput(password)
        composeTestRule.onNodeWithTag(SignInScreenTestTags.SIGN_IN_BUTTON)
            .assertExists()
        ...
        composeTestRule.onNodeWithTag("Add Discussion")
            .assertExists()
            .performClick()
        composeTestRule.waitForIdle()

        // Fill in discussion details
        composeTestRule.onNodeWithTag("Add Title")
            .assertExists()
            .performTextInput(discussionTitle)
        composeTestRule.onNodeWithTag("Add Description")
            .assertExists()
            .performTextInput("E2E test discussion")
        composeTestRule.waitForIdle()

        // Search for Bob and add him as a member during discussion creation
        composeTestRule.onNodeWithTag("Add Members", useUnmergedTree = true)
            .assertExists()
            .performTextInput(user2Handle)
        ...
        composeTestRule.waitUntil(timeoutMillis = 15_000) {
            try {
                composeTestRule.onNodeWithTag("Add Member Element", useUnmergedTree = true)
                    .assertExists()
                true
            } catch (_: Throwable) {
                false
        ...
        composeTestRule.onNodeWithTag("Create Discussion")
            .assertExists()
            .assertIsEnabled()
            .performClick()
        ...
        composeTestRule.onNodeWithText(discussionTitle, useUnmergedTree = true)
            .assertExists()
        ...
        composeTestRule.onNodeWithTag("Input Field", useUnmergedTree = true)
            .assertExists()
            .performTextInput(initialMessageFromAlice)

        composeTestRule.waitForIdle()

        composeTestRule.onNodeWithTag("Send Button")
            .assertExists()
            .performClick()
        ...
        composeTestRule.waitUntil(timeoutMillis = 15_000) {
            try {
                composeTestRule.onNodeWithText(initialMessageFromAlice, useUnmergedTree = true)
                    .assertExists()
                true
            } catch (_: Throwable) {
                false
        ...
        composeTestRule.onNodeWithTag(NavigationTestTags.GO_BACK_BUTTON, useUnmergedTree = true)
            .assertExists()
            .performClick()
