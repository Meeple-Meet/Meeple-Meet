...
class ThemeTestScreenInstrumentation {
    ...
    fun renderThemeTestScreenDark() {
        composeTestRule.setContent {
                ThemeTestScreen(themeMode = ThemeMode.DARK)
        }

        composeTestRule.waitForIdle()
        composeTestRule
            .onNodeWithTag("themeMode_DARK")
            .assertExists()
    ...
    fun renderThemeTestScreenLight() {
        composeTestRule.setContent {
                ThemeTestScreen(themeMode = ThemeMode.LIGHT)
        }

        composeTestRule.waitForIdle()
        composeTestRule
            .onNodeWithTag("themeMode_LIGHT")
            .assertExists()
        Thread.sleep(5000) // Keep the screen visible for 5 seconds
    }
    ...
    fun picksSystemTheme_whenNoArgumentProvided() {
        ...
        val darkExists = composeTestRule
            .onAllNodesWithTag("themeMode_DARK")
            .fetchSemanticsNodes().isNotEmpty()

        val lightExists = composeTestRule
            .onAllNodesWithTag("themeMode_LIGHT")
            .fetchSemanticsNodes().isNotEmpty()
